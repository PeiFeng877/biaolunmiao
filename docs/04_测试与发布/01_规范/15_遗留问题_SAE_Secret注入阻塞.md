...
## 0. 现行状态
- 本文件仅保留为历史问题快照。
- 2026-03-22 起，现行后端已切换为 `FC + FastAPI + 新购 RDS PostgreSQL Serverless`，旧 `SAE` 生产应用已删除。
- 任何新的 OSS 凭证/上传链路治理，必须以现行 FC 运行形态和现行发版规范为准，不再按本文执行。
...
## 变更日志
- 2026-03-22: 将本文标记为历史快照；旧 SAE 生产链路已删除，不再作为现行治理项。
