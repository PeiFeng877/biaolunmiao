...
- `02_数据库模型与迁移计划_v1.md`: 数据模型与迁移策略，覆盖 App 业务表、RDS PostgreSQL Serverless 目标态、队伍创建时间口径与测试账号命名规范。
- `03_错误码与响应规范.md`: 错误码、响应体与时间格式统一规则。

## 变更日志
- 2026-03-23: 队伍契约补充成员 `displayName` 与 `teams.member.update`，数据库模型新增 `team_members.display_name` 语义。
- 2026-03-23: 媒体上传契约补充正式客户端必须返回 `https` 媒体地址的约束，避免 iOS `ATS` 拦截。
- 2026-03-23: 队伍契约与数据模型补齐 `createdAt` 的客户端口径说明，供 iOS 多队场次编辑默认排序与回填使用。
