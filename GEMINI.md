...
- 设计一致性治理集中在 `docs/03_Governance`，通过脚本与 CI 阻断样式漂移。

## 当前阶段目标锁定（MVP 前置）

- 目标 1: 先把 UI 界面定稿（视觉风格、组件结构、页面布局先稳定）。
- 目标 2: 先把核心流程跑通（从入口到结果可闭环可演示）。
- 数据策略: 当前阶段只使用 Mock 数据，不以数据库结构约束前端设计。
- 设计约束: 先验证交互与信息密度，再反推字段与数据关系。
- 边界约束: 在 UI 与流程未冻结前，不锁定后端接口与数据库 Schema。
- 决策顺序: UI/流程冻结 -> 字段清单冻结 -> 外部交互冻结 -> 数据库与后端定版。

## 当前最重要的事情（按优先级）

- P0: 补齐并验证三条主流程闭环（队伍、赛事、日程），确保每条流程可从入口走到结果页面。
- P0: 清理“已展示但未生效”的操作（空按钮、占位动作、断链入口），避免伪完成状态。
- P1: 统一页面信息结构，产出稳定字段草案（必填/可选/展示级别），服务后续后端建模。
- P1: 固化 Mock 数据口径（样本覆盖正常/异常/边界场景），支撑持续 UI 回归。
- P2: 在流程稳定后再定义 API 边界与 DB 结构，避免提前固化错误抽象。

## 开发规范

- 结构变更必须同步更新各层 `GEMINI.md` 与文件头 `[PROTOCOL]`。
