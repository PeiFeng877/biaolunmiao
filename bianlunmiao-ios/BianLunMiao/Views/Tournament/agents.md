...
- `TournamentListView.swift`: 赛事列表主页，支持下拉刷新。
- `TournamentDetailView.swift`: 赛事详情容器与三段式场次编辑页。
- `Setup/*`: 赛事编排与发布流程页面。
- `Components/*`: 赛事复用视图组件。

## 变更日志
- 2026-03-23: 赛事详情右下角新建场次 FAB 收口到统一品牌交互色；场次编辑页“我方队伍”改为与“最佳辩手”一致的白底下拉选择器样式。
- 2026-02-08: 初始化赛事页面索引并引入 `Setup` 子模块，降低单层目录复杂度。
- 2026-03-23: 赛事列表根页接入下拉刷新，统一复用 `AppStore.refreshNow(force:)`。
- 2026-03-23: 赛事详情场次页切换为三段式编辑，并支持当前用户在可管理队伍中选择“我方队伍”。
